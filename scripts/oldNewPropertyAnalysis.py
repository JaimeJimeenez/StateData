...
county_year_variability = df_selected_county.groupBy("County", "Old/New").agg(stddev("price").alias("Variability"))

county_year_variability = county_year_variability.orderBy("County", "Old/New")
...
for county in selected_county:
    ...
    plt.plot(df_county["Old/New"], df_county["Variability"], label=county)

plt.xlabel('Viejo/Nuevo')
plt.ylabel('Desviación estándar según la variabilidad')
plt.title('Variabilidad del mercado de los condados según el estado de la propiedad')
